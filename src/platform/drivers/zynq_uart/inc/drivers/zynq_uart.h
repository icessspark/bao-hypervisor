...
#define UART_MODE_PAR_F0P \
    (0b010 << 3)  // Parity type select (forced to 0 Parity, Space Parity Mode)
#define UART_MODE_PAR_F1P \
    (0b011 << 3)  // Parity type select (forced to 1 Parity, Mark Parity Mode)
...
#define UART_MODE_CHRL_REF \
    (0b0 << 0)  // Clock source select (clock source is uart_ref_clk)
#define UART_MODE_CLKS_REF_8 \
    (0b1 << 0)  // Clock source select (clock source is uart_ref_clk/8)

/** UART Interrupt Enable Register configurations */

#define UART_ISR_EN_RBRK \
    (0b1 << 13)  // Receiver FIFO Break Detect interrupt (enable, clears mask=0)
#define UART_ISR_EN_TOVR \
    (0b1 << 12)  // Transmitter FIFO Overflow interrupt (enable, clears mask=0)
#define UART_ISR_EN_TNFUL \
    (0b1                  \
     << 11)  // Transmitter FIFO Nearly Full interrupt (enable, clears mask=0)
#define UART_ISR_EN_TTRIG \
    (0b1 << 10)  // Transmitter FIFO Trigger interrupt (enable, clears mask=0)
#define UART_ISR_EN_DMSI \
    (0b1                 \
     << 9)  // Delta Modem Status Indicator interrupt (enable, clears mask=0)
#define UART_ISR_EN_TIMEOUT \
    (0b1 << 8)  // Receiver Timeout Error interrupt (enable, clears mask=0)
#define UART_ISR_EN_PARE \
    (0b1 << 7)  // Receiver Parity Error interrupt (enable, clears mask=0)
#define UART_ISR_EN_FRAME \
    (0b1 << 6)  // Receiver Framing Error interrupt (enable, clears mask=0)
#define UART_ISR_EN_ROVR \
    (0b1 << 5)  // Receiver Overflow Error interrupt (enable, clears mask=0)
#define UART_ISR_EN_TFUL \
    (0b1 << 4)  // Transmitter FIFO Full interrupt (enable, clears mask=0)
#define UART_ISR_EN_TEMPTY \
    (0b1 << 3)  // Transmitter FIFO Empty interrupt (enable, clears mask=0)
#define UART_ISR_EN_RFUL \
    (0b1 << 2)  // Receiver FIFO Full interrupt (enable, clears mask=0)
#define UART_ISR_EN_REMPTY \
    (0b1 << 1)  // Receiver FIFO Empty interrupt (enable, clears mask=0)
#define UART_ISR_EN_RTRIG \
    (0b1 << 0)  // Receiver FIFO Trigger interrupt (enable, clears mask=0)

/** UART Interrupt Disable Register configurations */

#define UART_ISR_DIS_RBRK \
    (0b1 << 13)  // Receiver FIFO Break Detect interrupt (disable, sets mask=1)
#define UART_ISR_DIS_TOVR \
    (0b1 << 12)  // Transmitter FIFO Overflow interrupt (disable, sets mask=1)
#define UART_ISR_DIS_TNFUL \
    (0b1                   \
     << 11)  // Transmitter FIFO Nearly Full interrupt (disable, sets mask=1)
#define UART_ISR_DIS_TTRIG \
    (0b1 << 10)  // Transmitter FIFO Trigger interrupt (disable, sets mask=1)
#define UART_ISR_DIS_DMSI \
    (0b1 << 9)  // Delta Modem Status Indicator interrupt (disable, sets mask=1)
#define UART_ISR_DIS_TIMEOUT \
    (0b1 << 8)  // Receiver Timeout Error interrupt (disable, sets mask=1)
#define UART_ISR_DIS_PARE \
    (0b1 << 7)  // Receiver Parity Error interrupt (disable, sets mask=1)
#define UART_ISR_DIS_FRAME \
    (0b1 << 6)  // Receiver Framing Error interrupt (disable, sets mask=1)
#define UART_ISR_DIS_ROVR \
    (0b1 << 5)  // Receiver Overflow Error interrupt (disable, sets mask=1)
#define UART_ISR_DIS_TFUL \
    (0b1 << 4)  // Transmitter FIFO Full interrupt (disable, sets mask=1)
#define UART_ISR_DIS_TEMPTY \
    (0b1 << 3)  // Transmitter FIFO Empty interrupt (disable, sets mask=1)
#define UART_ISR_DIS_RFUL \
    (0b1 << 2)  // Receiver FIFO Full interrupt (disable, sets mask=1)
#define UART_ISR_DIS_REMPTY \
    (0b1 << 1)  // Receiver FIFO Empty interrupt (disable, sets mask=1)
#define UART_ISR_DIS_RTRIG \
    (0b1 << 0)  // Receiver FIFO Trigger interrupt (disable, sets mask=1)

/** UART Interrupt Mask Register configurations */

#define UART_ISR_MASK_RBRK \
    (0b1 << 13)  // Receiver FIFO Break Detect interrupt (enabled)
#define UART_ISR_MASK_TOVR \
    (0b1 << 12)  // Transmitter FIFO Overflow interrupt (enabled)
#define UART_ISR_MASK_TNFUL \
    (0b1 << 11)  // Transmitter FIFO Nearly Full interrupt (enabled)
#define UART_ISR_MASK_TTRIG \
    (0b1 << 10)  // Transmitter FIFO Trigger interrupt (enabled)
#define UART_ISR_MASK_DMSI \
    (0b1 << 9)  // Delta Modem Status Indicator interrupt (enabled)
#define UART_ISR_MASK_TIMEOUT \
    (0b1 << 8)  // Receiver Timeout Error interrupt (enabled)
#define UART_ISR_MASK_PARE \
    (0b1 << 7)  // Receiver Parity Error interrupt (enabled)
#define UART_ISR_MASK_FRAME \
    (0b1 << 6)  // Receiver Framing Error interrupt (enabled)
#define UART_ISR_MASK_ROVR \
    (0b1 << 5)  // Receiver Overflow Error interrupt (enabled)
#define UART_ISR_MASK_TFUL \
    (0b1 << 4)  // Transmitter FIFO Full interrupt (enabled)
#define UART_ISR_MASK_TEMPTY \
    (0b1 << 3)  // Transmitter FIFO Empty interrupt (enabled)
#define UART_ISR_MASK_RFUL (0b1 << 2)  // Receiver FIFO Full interrupt (enabled)
#define UART_ISR_MASK_REMPTY \
    (0b1 << 1)  // Receiver FIFO Empty interrupt (enabled)
#define UART_ISR_MASK_RTRIG \
    (0b1 << 0)  // Receiver FIFO Trigger interrupt (enabled)

/** UART Channel Interrupt Status Register configurations */

#define UART_ISR_STATUS_RBRK \
    (0b1 << 13)  // Receiver FIFO Break Detect interrupt (interrupt occured)
#define UART_ISR_STATUS_TOVR \
    (0b1 << 12)  // Transmitter FIFO Overflow interrupt (interrupt occurred)
#define UART_ISR_STATUS_TNFUL \
    (0b1 << 11)  // Transmitter FIFO Nearly Full interrupt (interrupt occurred)
#define UART_ISR_STATUS_TTRIG \
    (0b1 << 10)  // Transmitter FIFO Trigger interrupt (interrupt occurred)
#define UART_ISR_STATUS_DMSI \
    (0b1 << 9)  // Delta Modem Status Indicator interrupt (interrupt occurred)
#define UART_ISR_STATUS_TIMEOUT \
    (0b1 << 8)  // Receiver Timeout Error interrupt (interrupt occurred)
#define UART_ISR_STATUS_PARE \
    (0b1 << 7)  // Receiver Parity Error interrupt (interrupt occurred)
#define UART_ISR_STATUS_FRAME \
    (0b1 << 6)  // Receiver Framing Error interrupt (interrupt occurred)
#define UART_ISR_STATUS_ROVR \
    (0b1 << 5)  // Receiver Overflow Error interrupt (interrupt occurred)
#define UART_ISR_STATUS_TFUL \
    (0b1 << 4)  // Transmitter FIFO Full interrupt (interrupt occurred)
#define UART_ISR_STATUS_TEMPTY \
    (0b1 << 3)  // Transmitter FIFO Empty interrupt (interrupt occurred)
#define UART_ISR_STATUS_RFUL \
    (0b1 << 2)  // Receiver FIFO Full interrupt (interrupt occurred)
#define UART_ISR_STATUS_REMPTY \
    (0b1 << 1)  // Receiver FIFO Empty interrupt (interrupt occurred)
#define UART_ISR_STATUS_RTRIG \
    (0b1 << 0)  // Receiver FIFO Trigger interrupt (interrupt occurred)

/** UART Baud Rate Generator Register */

#define UART_BR_GEN_DIS \
    (0)  // Baud Rate Clock Divisor Value (Disables baud_sample)
#define UART_BR_GEN_BYPASS \
    (1)  // Baud Rate Clock Divisor Value (Clock divisor bypass)

/** UART Receiver Timeout Register */

#define UART_RX_TIMEOUT_DIS \
    (0)  // Receiver timeout value (Disables receiver timeout counter)

/** UART Receiver FIFO Trigger Level Register */

#define UART_RX_FIFO_TRIG_DIS \
    (0)  // RX FIFO trigger level value (Disables RX FIFO trigger level
         // function)
...
#define UART_MODEM_CTRL_RTS_FL1 \
    (0b0 << 1)  // Request to send output control (EMIOUARTxRTSN output forced
                // to logic 1)
#define UART_MODEM_CTRL_RTS_FL0 \
    (0b1 << 1)  // Request to send output control (EMIOUARTxRTSN output forced
                // to logic 0)
...
#define UART_MODEM_STATUS_CTS_H \
    (0b0 << 4)  // Clear to Send (CTS) input signal from PL(EMIOUARTxCTSN)
                // status (input is high)
#define UART_MODEM_STATUS_CTS_L \
    (0b1 << 4)  // Clear to Send (CTS) input signal from PL(EMIOUARTxCTSN)
                // status (input is low)
#define UART_MODEM_STATUS_DDCD \
    (0b1 << 3)  // Delta Data Carrier Detect status (change has occurred)
#define UART_MODEM_STATUS_TERI \
    (0b1 << 2)  // Trailing Edge Ring Indicator status (Trailing edge has
                // occurred)
#define UART_MODEM_STATUS_DDSR \
    (0b1 << 1)  // Delta Data Set Ready status (change has occurred)
#define UART_MODEM_STATUS_DCTS \
    (0b1 << 0)  // Delta Clear To Send status (change has occurred)
...
#define UART_TX_FIFO_TRIG_DIS \
    (0)  // TX FIFO trigger level value (Disables TX FIFO trigger level
         // function)
